%%%% Plot RAS nutrient and DIC/ALK data for quick look plots,
...
for k = 1:length(files)
    ...
    %plotVar={'NTRI_CONC','PHOS_CONC' ,'SLCA_CONC'};
    ...
    try
        ...
        p1 = plot(time(varQC <= 2), var(varQC <= 2),'.-','MarkerSize',5,'LineWidth',0.8,'DisplayName',plotVar{i}, 'Color', cl(i,:));
       
        hold on
        
        % plot the QC info
             
        plot(time(varQC == 2), var(varQC == 2), 'o', 'MarkerSize',6, 'MarkerFaceColor', cl(i,:), 'Color', cl(i, :));

        plot(time(varQC == 3), var(varQC == 3),'d','MarkerSize',6,'MarkerFaceColor', cl(i,:), 'Color', cl(i, :));

        plot(time(varQC == 4), var(varQC == 4),'s','MarkerSize', 6, 'MarkerFaceColor', cl(i,:), 'Color', cl(i, :))
...
%h= legend({'\color[rgb]{0 0.4470 0.7410}NO_{x}' '\color[rgb]{0.8500 0.3250 0.0980}Phosphate', '\color[rgb]{0.9290 0.6940 0.1250}Silicate'},'Orientation','horizontal','Location', 'southoutside','Interpreter', 'tex');
%set(h, 'FontSize', 10)
...
figurename = [files(1).folder '/' deployment_code '-RAS-nuts-report_plot_b.png'];
end
